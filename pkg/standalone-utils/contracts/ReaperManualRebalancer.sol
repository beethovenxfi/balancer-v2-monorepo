...
contract ReaperManualRebalancer {
    ...
    function wrap(
        ...
        (IERC20 mainToken, IERC20 wrappedToken, IReaperTokenVault vaultToken) = _getLinearPoolTokens(linearPoolId);
        ...
        _setReaperVaultAllowancesIfNeeded(mainToken, wrappedToken);

        for (uint256 i = 0; i < numLoops; ++i) {
            // Deposit the tokens into the vault
            vaultToken.deposit(mainToken.balanceOf(address(this)));
    ...
    function unwrap(
        ...
        for (uint256 i = 0; i < numLoops; ++i) {
            ...
            // Withdraw the vault tokens to receive main tokens
    ...
    function _getLinearPoolTokens(
        bytes32 linearPoolId
    ) internal view returns (IERC20 mainToken, IERC20 wrappedToken, IReaperTokenVault vaultToken) {
        ...
        vaultToken = IReaperTokenVault(address(wrappedToken));
    }

    function _setReaperVaultAllowancesIfNeeded(
        IERC20 mainToken,
        IERC20 wrappedToken
    ) internal {

...
describe('CTokenWrapping', function () {
  let dai: Token, cDAI: Contract;
  let user: SignerWithAddress, admin: SignerWithAddress;
  let vault: Vault;
  let relayer: Contract, relayerLibrary: Contract;
  const cDaiRate = fp(1.02);
  ...
  sharedBeforeEach('Deploy tokens and cTokens', async () => {
    dai = await Token.create({ name: 'DAI', symbol: 'DAI', decimals: 18 });
    cDAI = await deploy('v2-pool-linear/MockCToken', {
      args: ['cDAI', 'cDAI', 18, dai.address, cDaiRate],
    });
  });
  ...
  sharedBeforeEach('mint tokens to user and set allowances', async () => {
    ...
    await dai.approve(cDAI.address, fp(100), { from: user });
    await cDAI.connect(user).approve(vault.address, fp(100));
  });
  ...
  function encodeWrap(
    ...
    return relayerLibrary.interface.encodeFunctionData('wrapCToken', [
      vaultToken.address,
  ...
  function encodeUnwrap(
    ...
    return relayerLibrary.interface.encodeFunctionData('unwrapCToken', [
      vaultToken.address,
      TypesConverter.toAddress(sender),
      TypesConverter.toAddress(recipient),
      amount,
  ...
  describe('wrapping', () => {
    const amount = fp(1);
    const yvDaiForAmount = amount.mul(fp(1)).div(cDaiRate);

    it('should deposit underlying tokens into a reaper vault on wrap', async () => {
      const receipt = await (await relayer.connect(user).multicall([encodeWrap(cDAI, user, user, amount)])).wait();

      expectTransferEvent(receipt, { from: user.address, to: relayer.address, value: amount }, dai);
      expectTransferEvent(receipt, { from: ZERO_ADDRESS, to: relayer.address, value: yvDaiForAmount }, cDAI);
      expectTransferEvent(receipt, { from: relayer.address, to: user.address, value: yvDaiForAmount }, cDAI);
    });

    it('should leave yv tokens on the relayer, when the recipient of the wrap is the relayer', async () => {
      const receipt = await (
        await relayer.connect(user).multicall([encodeWrap(cDAI, user, relayer.address, amount)])
      ).wait();

      expectTransferEvent(receipt, { from: user.address, to: relayer.address, value: amount }, dai);

      const balance = await cDAI.balanceOf(relayer.address);

      expect(balance).to.be.equalWithError(yvDaiForAmount, 0.00001);
    });

    it('stores wrap output as chained reference', async () => {
      await relayer.connect(user).multicall([encodeWrap(cDAI, user, relayer.address, amount, toChainedReference(0))]);
    ...
    it('wraps with chained references', async () => {
      ...
      const receipt = await (
        await relayer.connect(user).multicall([encodeWrap(cDAI, user, relayer.address, toChainedReference(0))])
      ).wait();
  ...
  describe('unwrapping', () => {
    const amount = fp(1);
    const daiForAmount = amount.mul(cDaiRate).div(fp(1));

    sharedBeforeEach('deposit tokens to vault', async () => {
      await cDAI.connect(user).mintCTokens(daiForAmount);
    });

    it('should withdraw underlying tokens from a reaper vault on unwrap', async () => {
      const receipt = await (await relayer.connect(user).multicall([encodeUnwrap(cDAI, user, user, amount)])).wait();

      expectTransferEvent(receipt, { from: user.address, to: relayer.address, value: amount }, cDAI);
      expectTransferEvent(receipt, { from: cDAI.address, to: relayer.address, value: daiForAmount }, dai);
    ...
    it('should leave tokens on the relayer, when the recipient of the unwrap is the relayer', async () => {
      const receipt = await (
        await relayer.connect(user).multicall([encodeUnwrap(cDAI, user, relayer.address, amount)])
      ).wait();
    ...
    it('stores unwrap output as chained reference', async () => {
      await relayer.connect(user).multicall([encodeUnwrap(cDAI, user, relayer, amount, toChainedReference(0))]);

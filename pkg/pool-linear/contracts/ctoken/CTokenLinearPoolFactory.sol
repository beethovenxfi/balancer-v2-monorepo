...
import "./CTokenLinearPool.sol";
import "./CTokenLinearPoolRebalancer.sol";

contract CTokenLinearPoolFactory is ILastCreatedPoolFactory, BasePoolFactory, ReentrancyGuard, FactoryWidePauseWindow {
    ...
    ) BasePoolFactory(vault, protocolFeeProvider, type(CTokenLinearPool).creationCode) {
        _queries = queries;
    }
    ...
    /**
     * @dev Deploys a new `CTokenLinearPool`.
     */
    function create(
        string memory name,
        ...
        // We are going to deploy both an CTokenLinearPool and an CTokenLinearPoolRebalancer set as its Asset Manager,
        ...
        bytes memory rebalancerCreationCode = abi.encodePacked(
            type(CTokenLinearPoolRebalancer).creationCode,
        ...
        CTokenLinearPool.ConstructorArgs memory args = CTokenLinearPool.ConstructorArgs({
            vault: getVault(),
            name: name,
            symbol: symbol,
            mainToken: mainToken,
        ...
        CTokenLinearPool pool = CTokenLinearPool(_create(abi.encode(args)));

...
contract BooLinearPool is LinearPool, Version {
    ...
    {
        _xBOO = IBooMirrorWorld(address(args.wrappedToken));

        _require(
            address(args.mainToken) == address(IBooMirrorWorld(address(args.wrappedToken)).boo()),
    ...
    function _getWrappedTokenRate() internal view override returns (uint256) {
        ...
        try _xBOO.xBOOForBOO(1e18) returns (uint256 rate) {
            return rate;
        } catch (bytes memory revertData) {
            // By maliciously reverting here, BOO (or any other contract in the call stack) could trick the Pool into

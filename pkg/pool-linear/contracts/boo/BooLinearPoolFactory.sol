...
import "./BooLinearPool.sol";
import "./BooLinearPoolRebalancer.sol";

contract BooLinearPoolFactory is
    ...
    FactoryWidePauseWindow
{
    ...
    ) BasePoolFactory(vault, protocolFeeProvider, type(BooLinearPool).creationCode) Version(factoryVersion) {
        _queries = queries;
    ...
    /**
     * @dev Deploys a new `BooLinearPool`.
     */
    function create(
        string memory name,
        ...
        // We are going to deploy both an BooLinearPool and an BooLinearPoolRebalancer set as its Asset Manager, but
        ...
        bytes memory rebalancerCreationCode = abi.encodePacked(
            type(BooLinearPoolRebalancer).creationCode,
        ...
        BooLinearPool.ConstructorArgs memory args = BooLinearPool.ConstructorArgs({
            vault: getVault(),
            name: name,
            symbol: symbol,
            mainToken: mainToken,
        ...
        BooLinearPool pool = BooLinearPool(_create(abi.encode(args)));

...
import "./TarotLinearPool.sol";
import "./TarotLinearPoolRebalancer.sol";

contract TarotLinearPoolFactory is
    ...
    FactoryWidePauseWindow
{
    ...
    ) BasePoolFactory(vault, protocolFeeProvider, type(TarotLinearPool).creationCode) Version(factoryVersion) {
        _queries = queries;
    ...
    /**
     * @dev Deploys a new `TarotLinearPool`.
     */
    function create(
        string memory name,
        ...
        // We are going to deploy both an TarotLinearPool and an TarotLinearPoolRebalancer set as its Asset Manager, but
        ...
        bytes memory rebalancerCreationCode = abi.encodePacked(
            type(TarotLinearPoolRebalancer).creationCode,
        ...
        TarotLinearPool.ConstructorArgs memory args = TarotLinearPool.ConstructorArgs({
            vault: getVault(),
            name: name,
            symbol: symbol,
            mainToken: mainToken,
        ...
        TarotLinearPool pool = TarotLinearPool(_create(abi.encode(args)));

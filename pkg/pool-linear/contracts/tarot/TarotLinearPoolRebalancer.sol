...
import "./TarotShareValueHelper.sol";

contract TarotLinearPoolRebalancer is LinearPoolRebalancer, TarotShareValueHelper {
    ...
    constructor(IVault vault, IBalancerQueries queries)
        ...
        TarotShareValueHelper(ILinearPool(ILastCreatedPoolFactory(msg.sender).getLastCreatedPool()).getWrappedToken())
    {
    ...
    function _wrapTokens(uint256 amount) internal override {
        ...
        ISushiBar(address(_wrappedToken)).enter(amount);
    }
    ...
    function _unwrapTokens(uint256 amount) internal override {
        ...
        ISushiBar(address(_wrappedToken)).leave(amount);
    }

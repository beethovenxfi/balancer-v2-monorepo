...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  /* if (task.mode === TaskMode.LIVE) {
    ...
    if (force || !task.output({ ensure: false })['MockMidasLinearPool']) {
      ...
      await task.save({ MockMidasLinearPool: mockPoolAddress });
    }

    const mockPool = await task.instanceAt('MidasLinearPool', task.output()['MockMidasLinearPool']);
    ...
    await task.verify('MidasLinearPool', mockPool.address, [mockPoolArgs]);

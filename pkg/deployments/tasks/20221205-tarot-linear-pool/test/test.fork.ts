...
describeForkTest('TarotLinearPoolFactory', 'opera', 51905777, function () {
  ...
  const tUSDC = '0x68d211Bc1e66814575d89bBE4F352B4cdbDACDFb';

  const USDC_SCALING = bn(1e12); // USDC has 6 decimals, so its scaling factor is 1e12

  const USDC_HOLDER = '0x12edea9cd262006cc3c4e77c90d2cd2dd4b1eb97';
  ...
  describe('create, join, and rebalance', () => {
    it('deploy a linear pool', async () => {
      const tx = await factory.create('', '', USDC, tUSDC, INITIAL_UPPER_TARGET, SWAP_FEE_PERCENTAGE, owner.address);
      const event = expectEvent.inReceipt(await tx.wait(), 'PoolCreated');

      pool = await task.instanceAt('TarotLinearPool', event.args.pool);
      ...
      rebalancer = await task.instanceAt('TarotLinearPoolRebalancer', assetManager);

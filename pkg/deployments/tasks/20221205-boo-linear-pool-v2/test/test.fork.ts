...
describeForkTest('BooLinearPoolFactory', 'opera', 51905777, function () {
  let owner: SignerWithAddress, holder: SignerWithAddress, other: SignerWithAddress;
  let factory: Contract, vault: Contract, boo: Contract;
  ...
  const BOO_SCALING = bn(1e0); // USDC has 6 decimals, so its scaling factor is 1e12

  const BOO_HOLDER = '0xf778f4d7a14a8cb73d5261f9c61970ef4e7d7842';
  ...
  before('run task', async () => {
    task = new Task('20221205-boo-linear-pool', TaskMode.TEST, getForkedNetwork(hre));
    await task.run({ force: true });
    factory = await task.deployedInstance('BooLinearPoolFactory');
  });

  before('load signers', async () => {
    [, owner, other] = await getSigners();

    holder = await impersonate(BOO_HOLDER, fp(100));
  });

  before('setup contracts', async () => {
    vault = await new Task('20210418-vault', TaskMode.READ_ONLY, getForkedNetwork(hre)).deployedInstance('Vault');

    boo = await task.instanceAt('IERC20', BOO);
    await boo.connect(holder).approve(vault.address, MAX_UINT256);
  });
  ...
  function itRebalancesThePool(expectedState: LinearPoolState) {
    it('rebalance the pool', async () => {
      ...
      const { cash } = await vault.getPoolTokenInfo(poolId, BOO);
      const scaledCash = cash.mul(BOO_SCALING);
      ...
      const initialRecipientMainBalance = await boo.balanceOf(other.address);
      if (expectedState != LinearPoolState.BALANCED) {
        await rebalancer.connect(holder).rebalance(other.address);
      } else {
        await rebalancer.connect(holder).rebalanceWithExtraMain(other.address, 55);
      }
      const finalRecipientMainBalance = await boo.balanceOf(other.address);
      ...
      const mainInfo = await vault.getPoolTokenInfo(poolId, BOO);

      const expectedMainBalance = lowerTarget.add(upperTarget).div(2);
      expect(mainInfo.cash.mul(BOO_SCALING)).to.equal(expectedMainBalance);
  ...
  describe('create, join, and rebalance', () => {
    it('deploy a linear pool', async () => {
      const tx = await factory.create('', '', BOO, xBOO, INITIAL_UPPER_TARGET, SWAP_FEE_PERCENTAGE, owner.address);
      const event = expectEvent.inReceipt(await tx.wait(), 'PoolCreated');

      pool = await task.instanceAt('BooLinearPool', event.args.pool);
      ...
      const { assetManager } = await vault.getPoolTokenInfo(poolId, BOO); // We could query for either USDC or waUSDC
      rebalancer = await task.instanceAt('BooLinearPoolRebalancer', assetManager);

      await boo.connect(holder).approve(rebalancer.address, MAX_UINT256); // To send extra main on rebalance
    });
    ...
    it('join the pool', async () => {
      ...
      const joinAmount = INITIAL_UPPER_TARGET.mul(2).div(BOO_SCALING);

      await vault
        .connect(holder)
        .swap(
          { kind: SwapKind.GivenIn, poolId, assetIn: BOO, assetOut: pool.address, amount: joinAmount, userData: '0x' },
      ...
      const excess = joinAmount.mul(BOO_SCALING).sub(INITIAL_UPPER_TARGET);
      const joinCollectedFees = excess.mul(SWAP_FEE_PERCENTAGE).div(FP_ONE);

      const expectedBPT = joinAmount.mul(BOO_SCALING).sub(joinCollectedFees);
  ...
  describe('generate excess of main token and rebalance', () => {
    it('deposit main tokens', async () => {
      ...
      const joinAmount = upperTarget.mul(5).div(BOO_SCALING);
  ...
  describe('generate lack of main token and rebalance', () => {
    it('withdraw main tokens', async () => {
      ...
      const exitAmount = scaledCash.sub(lowerTarget.div(3)).div(BOO_SCALING);
  ...
  describe('join below upper target and rebalance', () => {
    it('deposit main tokens', async () => {
      ...
      const joinAmount = midpoint.div(100).div(BOO_SCALING);
  ...
  describe('exit above lower target and rebalance', () => {
    it('withdraw main tokens', async () => {
      ...
      const exitAmount = midpoint.div(100).div(BOO_SCALING);
